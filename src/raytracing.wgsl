...
fn is_puddle_candidate(id: u32) -> bool {
    switch id {
        ...
        case 27u: { return true; }  // diamond ore
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    let day = smoothstep(-0.07, 0.18, sun_height);
    let night = 1.0 - day;
    let ambient_boost = clamp(u.lighting.x, 0.5, 2.0);
    let sun_softness = clamp(u.lighting.y, 0.0, 1.0);
    let fog_mul = clamp(u.lighting.z, 0.1, 3.0);
    let exposure = clamp(u.lighting.w, 0.5, 2.5);
    let sun_dir = normalize(vec3<f32>(cos(sun_angle), max(sun_height, 0.0), 0.35));
    let moon_dir = -sun_dir;

    let sun_wrap = mix(0.03, 0.30, sun_softness);
    let ndl = clamp((dot(normal, sun_dir) + sun_wrap) / (1.0 + sun_wrap), 0.0, 1.0);
    ...
    let face_light = mix(face_base, hemi_soft, 0.30 + sun_softness * 0.42);
    let underground = clamp((40.0 - hit_world.y) / 48.0, 0.0, 1.0);
    let cave_dim = mix(1.0, 0.62, underground);
    var shadow = 1.0;
    let sun_vis = day * ndl * (1.0 - underground * 0.78);
    if sun_vis > 0.02 {
        shadow = trace_shadow(
    ...
    let ambient_scalar = mix(0.20, 0.72, day) * cave_dim * ambient_boost;
    let sky_fill = mix(0.04, 0.20, day)
        * (0.45 + 0.55 * clamp(normal.y * 0.5 + 0.5, 0.0, 1.0))
        * mix(1.0, 0.74, underground)
        * mix(0.88, 1.08, clamp(ambient_boost * 0.5, 0.0, 1.0));
    let sun_scalar = (0.22 + 0.78 * pow(ndl, 1.30)) * day * shadow;
    let moon_scalar = (0.03 + 0.14 * pow(nml, 1.25)) * night * 0.72;
    let light_scalar = clamp(face_light * (ambient_scalar + sun_scalar + moon_scalar) + sky_fill, 0.04, 1.75);

    let tint = mix(vec3<f32>(0.80, 0.88, 1.08), vec3<f32>(1.03, 1.00, 0.96), day);
    let lit_linear = max(base * light_scalar * tint * exposure, vec3<f32>(0.0));
    let lit_tm = tonemap_reinhard(lit_linear);
    let lit_luma = dot(lit_tm, vec3<f32>(0.2126, 0.7152, 0.0722));
    let lit_sat = mix(vec3<f32>(lit_luma), lit_tm, 1.06);
    let lit = pow(lit_sat, vec3<f32>(1.0 / 2.15));
    var shaded = lit;
    ...
    let dist = t;
    let fog_density = mix(0.0020, 0.00088, day) * fog_mul;
    let fog = clamp(1.0 - exp(-dist * fog_density), 0.0, 1.0);
    let sky = sky_color(day_time, rd_world.y);
    let color = mix(shaded, sky, fog * mix(0.90, 0.78, day));
