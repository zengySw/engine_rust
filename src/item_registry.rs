...
const SLOW_UNSUPPORTED_MINE_SECONDS: f32 = 8.0;
...
pub fn block_break_seconds(block: Block, tool: Option<WoodenTool>) -> f32 {
    ...
    if is_pickaxe_harvest_block(block) && !can_harvest_block(block, tool) {
        return SLOW_UNSUPPORTED_MINE_SECONDS;
    }
    let speed_mult = tool.map_or(1.0, |t| global().tool_break_multiplier(t, block));
    (base / speed_mult.max(0.1)).max(0.05)
}

pub fn can_harvest_block(block: Block, tool: Option<WoodenTool>) -> bool {
    match block {
        Block::Stone | Block::CoalOre | Block::CopperOre | Block::Furnace => {
            is_pickaxe(tool)
        }
        Block::IronOre => matches!(tool, Some(WoodenTool::StonePickaxe | WoodenTool::IronPickaxe)),
        Block::DiamondOre => matches!(tool, Some(WoodenTool::IronPickaxe)),
        _ => true,
    }
}
...
fn is_pickaxe_harvest_block(block: Block) -> bool {
    matches!(
        block,
        Block::Stone
            | Block::CoalOre
...
fn is_pickaxe(tool: Option<WoodenTool>) -> bool {
    matches!(
        tool,
        Some(WoodenTool::Pickaxe | WoodenTool::StonePickaxe | WoodenTool::IronPickaxe)
    )
}
