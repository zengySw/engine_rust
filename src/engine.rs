...
use crate::save;
use crate::save::WorldType;
use crate::sound::SoundSystem;
use crate::start_menu::{StartMenu, StartMenuAction};
use crate::world::block::Block;
use crate::world::biome::{self, Biome};
use crate::world::chunk::{spawn_point, CHUNK_D, CHUNK_H, CHUNK_W};
...
const CAVE_MOOD_INCREASE_MIN: f32 = 0.25;
const CAVE_MOOD_INCREASE_MAX: f32 = 1.10;
const CAVE_MOOD_DECAY: f32 = 2.2;
...
// RTX volume/ray state refresh at 24Hz to reduce spikes on mid-range GPUs.
const RTX_UPDATE_INTERVAL_SECS: f32 = 1.0 / 24.0;
...
impl Engine {
    pub async fn new(args: Args) -> (Self, EventLoop<()>) {
        ...
        let initial_world_type = save::world_type_for_seed(world_seed);
        let (spawn_x, spawn_y, spawn_z) = spawn_point(world_seed, initial_world_type);
        ...
        let start_menu = StartMenu::new(args.world_file.is_none());
        ...
        let mut engine = Self {
            ...
            rt_update_timer: 0.0,
            last_saved_settings: settings,
            current_world_type: initial_world_type,
        };
        ...
        engine.ensure_world_meta_defaults();
        ...
        event_loop.run(move |event, elwt| {
            match &event {
                        ...
                        match we {
                            ...
                            WindowEvent::MouseInput { state, button, .. } => {
                                if *button == MouseButton::Left {
                                    if !self.menu.open && !self.inventory.open && !self.start_menu.open {
                            ...
                            WindowEvent::MouseWheel { delta, .. } => {
                                if !self.menu.open && !self.start_menu.open {
                ...
                Event::DeviceEvent {
                    ...
                    if self.focused && !self.menu.open && !self.inventory.open && !self.start_menu.open {
                ...
                Event::AboutToWait => {
                    ...
                    let block_outline = if !self.menu.open && !self.inventory.open && !self.start_menu.open {
                        self.collect_block_outline_visual()
                    } else {
                        None
                    };
                    self.renderer.update_block_outline(block_outline);
                    let hand_visual = if !self.third_person
                        && !self.menu.open
                        && !self.inventory.open
                        && !self.start_menu.open
                    {
                    ...
                    let target_hud = if !self.menu.open && !self.inventory.open && !self.start_menu.open {
                    ...
                    let start_menu = &mut self.start_menu;
                    ...
                    let mut start_menu_action = StartMenuAction::None;

                    match self.renderer.render(&self.camera, |ctx| {
                        // FPS оверлей (когда меню закрыто)
                        if show_debug && !menu.open && !inventory.open && !start_menu.open {
                            if let Some(data) = debug_data.as_ref() {
                                draw_f3_overlay(ctx, data);
                            }
                        } else if show_fps && !menu.open && !start_menu.open {
                        ...
                        // Меню паузы
                        start_menu_action = start_menu.draw(ctx, &mut menu.settings);
                    ...
                    match start_menu_action {
                        StartMenuAction::LoadWorld { seed, path } => {
                            self.load_world(seed, Some(path), true);
                        }
                    ...
                    if !self.menu.open && !self.start_menu.open && !self.focused {
                        self.grab_cursor(true);
                        self.focused = true;
                    }
    ...
    fn regenerate_world(&mut self, seed_override: Option<u32>) {
        let seed = seed_override.unwrap_or_else(|| next_seed(self.world_seed));
        self.load_world(seed, None, false);
    }

    fn load_world(&mut self, seed: u32, world_path: Option<PathBuf>, from_start_menu: bool) {
        self.save_player_state();
        self.save_inventory_state();
        self.world.save_all();

        save::clear_world_file_override(self.world_seed);
        save::clear_world_type_override(self.world_seed);
        if let Some(path) = world_path {
            save::set_world_file_override(seed, path);
        } else {
            save::clear_world_file_override(seed);
        }

        let world_type = save::world_type_for_seed(seed);
        save::set_world_type_override(seed, world_type);
        ...
        let (spawn_x, spawn_y, spawn_z) = spawn_point(self.world_seed, self.current_world_type);
        ...
        self.ensure_world_meta_defaults();
    ...
    fn ensure_world_meta_defaults(&mut self) {
        let existing = save::load_world_meta(self.world_seed);
        let mut meta = existing.unwrap_or_else(|| save::default_world_meta(
            self.world_seed,
            format!("world_{}", self.world_seed),
        ));
        meta.seed = self.world_seed;
        meta.world_type = self.current_world_type;
        if meta.name.trim().is_empty() {
            meta.name = format!("world_{}", self.world_seed);
        }
        save::save_world_meta(self.world_seed, &meta);
    }
    ...
    fn update_block_break_progress(&mut self, target: (i32, i32, i32), face: (i32, i32, i32), dt: f32) -> bool {
        ...
        if item_registry::can_harvest_block(broken, self.equipped_tool) {
    ...
    fn update_ambient_audio(&mut self, dt: f32) {
        ...
        let under_roof = self.player_is_under_roof();
        if let Some(sound) = self.sound_system.as_mut() {
            sound.update_rain_loop(self.rain_strength, under_roof);
        }
    ...
    fn player_is_under_roof(&self) -> bool {
        let eye = self.player.eye_pos();
        let wx = eye.x.floor() as i32;
        let wz = eye.z.floor() as i32;
        let mut y = eye.y.floor() as i32 + 1;
        if y < 0 {
            y = 0;
        }
        while y < CHUNK_H as i32 {
            if self.world.block_at_world(wx, y, wz).is_solid() {
                return true;
            }
            y += 1;
        }
    ...
    fn update_cave_mood(&mut self, dt: f32) {
        ...
        if self.cave_mood_cooldown > 0.0 {
            self.cave_mood_percent = 0.0;
            return;
        }
        ...
        if self.cave_mood_percent < CAVE_MOOD_TRIGGER {
            return;
        }
    ...
    fn respawn_player(&mut self) {
        let (sx, sy, sz) = spawn_point(self.world_seed, self.current_world_type);
