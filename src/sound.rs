...
pub struct SoundSystem {
    ...
    ambient_volume: f32,
    sfx_volume: f32,
    rain_loop_sink: Option<Sink>,
    rain_loop_path: Option<PathBuf>,
    rain_loop_indoors: bool,
    rain_loop_volume: f32,
}
...
impl SoundSystem {
    pub fn new() -> Option<Self> {
        ...
        if break_sounds.is_empty() {
            log::warn!("No break sounds found (expected *_break.(mp3/ogg/wav/flac) or block/<family>/break/*)");
        }
        if walk_sounds.is_empty() {
            log::warn!("No walk sounds found (expected *_walk.(mp3/ogg/wav/flac) or block/<family>/walk/*)");
        }
        ...
        Some(Self {
            ...
            rain_loop_sink: None,
            rain_loop_path: None,
            rain_loop_indoors: false,
            rain_loop_volume: 0.0,
        })
    }
    ...
    pub fn update_rain_loop(&mut self, rain_strength: f32, indoors: bool) {
        const RAIN_LOOP_SLICE_SECS: f32 = 1.0;
        ...
        let families = if indoors {
            &["rain_indoor", "rain_inside", "rain_muffled", "rain_room", "rain"][..]
        } else {
            &["rain_outdoor", "rain_surface", "rain"][..]
        };

        let Some(path) = choose_loop_sound_any(
            &self.misc_sounds,
            &self.break_sounds,
            &self.walk_sounds,
            families,
        ...
        let needs_restart = self
            .rain_loop_path
            .as_ref()
            .map_or(true, |p| p != &path)
            || self.rain_loop_sink.is_none()
            || self.rain_loop_indoors != indoors;
        if needs_restart {
            self.stop_rain_loop();
            let file = match File::open(&path) {
                Ok(f) => f,
                Err(err) => {
                    log::warn!("Failed to open rain loop sound {:?}: {}", path, err);
                    return;
                }
            };
            let decoder = match Decoder::new(BufReader::new(file)) {
                Ok(d) => d,
                Err(err) => {
                    log::warn!("Failed to decode rain loop sound {:?}: {}", path, err);
                    return;
                }
            };
            let sink = match Sink::try_new(&self.handle) {
                Ok(s) => s,
                Err(err) => {
                    log::warn!("Failed to create rain loop sink: {}", err);
                    return;
                }
            };
            sink.set_volume(0.0);
            if indoors {
                // Indoor rain should feel dampened and distant.
                sink.append(
                    decoder
                        .take_duration(Duration::from_secs_f32(RAIN_LOOP_SLICE_SECS))
                        .speed(0.88)
                        .repeat_infinite(),
                );
            } else {
            ...
            sink.play();
            self.rain_loop_sink = Some(sink);
            self.rain_loop_path = Some(path);
            self.rain_loop_indoors = indoors;
            self.rain_loop_volume = 0.0;
        }

        let base = if indoors {
            (0.015 + rain_strength * 0.10).clamp(0.01, 0.14)
        } else {
            (0.10 + rain_strength * 0.52).clamp(0.06, 0.68)
        };
        let target = self.mix_volume(base, SoundBus::Ambient);
        self.rain_loop_volume += (target - self.rain_loop_volume) * 0.22;
        if let Some(sink) = self.rain_loop_sink.as_ref() {
            sink.set_volume(self.rain_loop_volume.max(0.0));
        }
    }

    fn stop_rain_loop(&mut self) {
        if let Some(sink) = self.rain_loop_sink.take() {
            sink.stop();
        }
        self.rain_loop_path = None;
        self.rain_loop_indoors = false;
        self.rain_loop_volume = 0.0;
    }
...
fn choose_loop_sound_any(
    misc: &HashMap<String, Vec<PathBuf>>,
    break_map: &HashMap<String, Vec<PathBuf>>,
    walk_map: &HashMap<String, Vec<PathBuf>>,
    families: &[&str],
    prefer_last_variant: bool,
) -> Option<PathBuf> {
    let pick = |paths: &Vec<PathBuf>| -> Option<PathBuf> {
        if paths.is_empty() {
            None
        } else if prefer_last_variant {
            paths.last().cloned()
        } else {
            paths.first().cloned()
        }
    ...
    for family in families {
        if let Some(paths) = misc.get(*family) {
            if let Some(path) = pick(paths) {
                return Some(path);
            }
        }
    }
    for family in families {
        if let Some(paths) = break_map.get(*family) {
            if let Some(path) = pick(paths) {
                return Some(path);
            }
        }
    ...
    for family in families {
        if let Some(paths) = walk_map.get(*family) {
            if let Some(path) = pick(paths) {
                return Some(path);
            }
        }
...
fn collect_audio_files_recursive(dir: &Path, out: &mut Vec<PathBuf>) {
...
fn break_sound_families(block: Block) -> &'static [&'static str] {
    match block {
        ...
        | Block::IronOre
        | Block::CopperOre
        | Block::DiamondOre => &["stone", "gravel", "sand", "grass", "break"],
    }
}
...
fn walk_sound_families(block: Block) -> &'static [&'static str] {
    match block {
        ...
        | Block::IronOre
        | Block::CopperOre
        | Block::DiamondOre => &["stone", "gravel", "sand", "wood"],
    }
}
