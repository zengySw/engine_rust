...
fn generate_flat_chunk(cx: i32, cz: i32) -> Chunk {
    ...
    for x in 0..CHUNK_W {
        for z in 0..CHUNK_D {
            ...
            blocks[idx(x, 1, z)] = Block::Dirt;
            blocks[idx(x, 2, z)] = Block::Dirt;
            blocks[idx(x, 3, z)] = Block::Dirt;
            blocks[idx(x, 4, z)] = Block::Grass;
            hmap.surface[x][z] = 4;
            hmap.biome[x][z] = Biome::Plains;
        }
    }
...
fn apply_broken_world_spacing(blocks: &mut [Block], cx: i32, cz: i32) {
    for x in 0..CHUNK_W {
...
pub fn spawn_point(seed: u32, world_type: WorldType) -> (f32, f32, f32) {
    if world_type == WorldType::Flat {
        return (0.5, 6.05, 0.5);
    }
    let sampler = TerrainSampler::new(seed);
    ...
    if world_type == WorldType::Broken {
        (sx + 0.5, sy, sz + 0.5)
    } else {
        (sx, sy, sz)
    }
...
impl Chunk {
    pub fn generate(cx: i32, cz: i32, seed: u32, world_type: WorldType) -> Self {
        if world_type == WorldType::Flat {
        ...
        let ore_diamond = Perlin::new(seed.wrapping_add(34));
        let size = CHUNK_W * CHUNK_H * CHUNK_D;
        let mut blocks = vec![Block::Air; size].into_boxed_slice();
        ...
        if world_type == WorldType::Broken {
            apply_broken_world_spacing(&mut blocks, cx, cz);
        }
        Self {
            cx,
            cz,
...
fn face_texture_for(block: Block, face_idx: usize) -> u32 {
    ...
    match block {
        ...
        Block::Wood => Block::Wood.texture_index(),
        Block::Workbench => {
...
fn place_boulders(blocks: &mut [Block], cx: i32, cz: i32, seed: u32, hmap: &Heightmap) {
    ...
    for x in 1..(CHUNK_W - 1) {
        ...
        for z in 1..(CHUNK_D - 1) {
            ...
            let boulder_block = if ore_roll < 0.016 {
                Block::CoalOre
            } else if ore_roll < 0.022 {
                Block::IronOre
            } else if ore_roll < 0.027 {
                Block::CopperOre
            } else {
                Block::Stone
            };
...
fn pick_stone_block(
    ...
    // Vein-like ore placement (sparser/smaller):
    // combine higher-frequency primary noise with stronger ridge suppression.
    let wxf = wx as f64;
    let wyf = wy as f64;
    let wzf = wz as f64;
    ...
    let depth_from_surface = (surface_y as i32 - wy).max(0) as f64;
    let mountain_bonus = if biome::is_mountain(biome) { 0.02 } else { 0.0 };

    // Iron veins: deep, but much sparser and tighter than before.
    if wy <= 66 {
        let primary = ore_iron.get([vx / 11.0, wyf / 9.0, vz / 11.0]);
        let ridge = ore_warp.get([vx / 4.9 + 41.0, wyf / 4.9 - 13.0, vz / 4.9 + 9.0]).abs();
        let signal = primary - ridge * 0.61;
        let depth = (1.0 - (wyf / 66.0)).clamp(0.0, 1.0);
        let threshold = 0.76 - depth * 0.08 - mountain_bonus * 0.35;
        if signal > threshold {
            return Block::IronOre;
        }
    }

    // Copper veins: mid-depth, also reduced strongly.
    if wy <= 82 {
        let primary = ore_copper.get([vx / 12.0 + 13.0, wyf / 8.0, vz / 12.0 - 17.0]);
        let ridge = ore_warp.get([vx / 5.1 - 5.0, wyf / 5.1 + 27.0, vz / 5.1 + 3.0]).abs();
        let mid_bias = 1.0 - ((wyf - 34.0).abs() / 34.0).clamp(0.0, 1.0);
        let signal = primary - ridge * 0.58;
        let threshold = 0.79 - mid_bias * 0.10 - mountain_bonus * 0.25;
        if signal > threshold {
            return Block::CopperOre;
        }
    }

    // Coal veins: still most common ore, but way less dense than before.
    if wy <= 120 {
        let primary = ore_coal.get([vx / 13.0 - 9.0, wyf / 10.0, vz / 13.0 + 21.0]);
        let ridge = ore_warp.get([vx / 5.8 + 33.0, wyf / 5.8 + 7.0, vz / 5.8 - 15.0]).abs();
        let signal = primary - ridge * 0.53;
        let deep_bias = (1.0 - (wyf / 120.0)).clamp(0.0, 1.0);
        let threshold = 0.73 - deep_bias * 0.08 - mountain_bonus * 0.25;
        if signal > threshold {
    ...
    // Diamond ore:
    // world uses local surface as "Y=0" reference for depth gameplay:
    // spawn only when we are at least 36 blocks below local surface.
    if depth_from_surface >= 36.0 {
        let primary = ore_diamond.get([vx / 9.0 + 73.0, wyf / 6.4 - 31.0, vz / 9.0 + 57.0]);
        let ridge = ore_warp.get([vx / 4.0 + 91.0, wyf / 4.0 - 19.0, vz / 4.0 + 27.0]).abs();
        let deep_bonus = ((depth_from_surface - 36.0) / 40.0).clamp(0.0, 0.10);
        let signal = primary - ridge * 0.72;
        let threshold = 0.93 - deep_bonus;
        if signal > threshold {
    ...
    if depth_from_surface >= 36.0 && r < 0.000035 {
        return Block::DiamondOre;
    }
