...
struct DragDistributeState {
    ...
    // For primary-drag we place items immediately while cursor moves.
    // Keep per-slot provisional adds so we can rebalance when new slots are touched.
    placed_amounts: Vec<(SlotId, u16)>,
}
...
impl Inventory {
                ...
                frame.show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        for i in 0..HOTBAR_SLOTS {
                            ...
                            let durability_ratio = self.slot_tool_durability_ratio(slot);
                            let resp = paint_slot(
                                ui,
                                &slot,
                                i == self.selected,
                                            ...
                                            ui.horizontal(|ui| {
                                                for col in 0..craft_cols {
                                                    ...
                                                    let durability_ratio =
                                                        self.slot_tool_durability_ratio(slot);
                                                    let resp = paint_slot(
                                                        ui,
                                                        &slot,
                                                        false,
    ...
    fn draw_furnace_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        ui.horizontal(|ui| {
            ui.vertical(|ui| {
                ...
                let input_ratio = self.slot_tool_durability_ratio(self.furnace_input);
                let input_resp = paint_slot(
                    ui,
                    &self.furnace_input,
                    false,
                    slot_size,
                ...
                let fuel_ratio = self.slot_tool_durability_ratio(self.furnace_fuel);
                let fuel_resp = paint_slot(
                    ui,
                    &self.furnace_fuel,
                    false,
                    slot_size,
            ...
            ui.vertical(|ui| {
                ...
                let out_ratio = self.slot_tool_durability_ratio(self.furnace_output);
                let out_resp = paint_slot(
                    ui,
                    &self.furnace_output,
                    false,
                    slot_size,
    ...
    fn handle_slot_interaction(&mut self, slot_id: SlotId, resp: &egui::Response) {
        ...
        // Secondary drag places one per touched slot immediately.
        // Primary drag rebalances evenly across all touched slots live.
    ...
    fn try_start_drag_distribution(
        ...
        self.drag_distribute = Some(DragDistributeState {
            button,
            carried_block: self.carried.block,
            touched_slots: Vec::new(),
            placed_amounts: Vec::new(),
        });
    ...
    fn touch_drag_slot(&mut self, slot_id: SlotId) {
        ...
        if button == DragDistributeButton::Secondary {
            ...
            self.rebalance_primary_drag_distribution();
        }
    }
    ...
    fn finish_drag_distribution_if_released(&mut self, ctx: &egui::Context) {
        ...
        if state.button == DragDistributeButton::Primary {
            // Primary drag is applied live while moving.
            // Nothing additional is required on release.
            if state.placed_amounts.is_empty() && state.touched_slots.len() == 1 {
                self.primary_click_slot(state.touched_slots[0]);
            }
        }
    }

    fn rebalance_primary_drag_distribution(&mut self) {
        let (carried_block, touched_slots, prev_placed) = {
            let Some(state) = self.drag_distribute.as_mut() else {
                return;
            };
            if state.button != DragDistributeButton::Primary {
                return;
            ...
            (
                state.carried_block,
                state.touched_slots.clone(),
                std::mem::take(&mut state.placed_amounts),
            )
        };

        // Rollback previous provisional placement to re-apply with updated slot set.
        for (slot_id, amount) in prev_placed {
            if amount == 0 || self.carried.is_tool() {
                continue;
            }
            let slot = self.slot_mut(slot_id);
            if slot.is_empty() || slot.is_tool() || slot.block != carried_block {
                continue;
            }
            let take = amount.min(slot.count);
            slot.count = slot.count.saturating_sub(take);
            if slot.count == 0 {
                slot.clear();
            }
            self.carried.count = self.carried.count.saturating_add(take).min(MAX_STACK_SIZE);
            self.carried.block = carried_block;
            self.carried.tool = None;
        }
        ...
        let mut new_placed: Vec<(SlotId, u16)> = Vec::new();
        ...
        for i in 0..valid_slots.len() {
            ...
            push_drag_placed_amount(&mut new_placed, valid_slots[i], put);
        ...
        while !self.carried.is_empty() {
            ...
            for i in 0..valid_slots.len() {
                ...
                push_drag_placed_amount(&mut new_placed, valid_slots[i], 1);
        ...
        if let Some(state) = self.drag_distribute.as_mut() {
            if state.button == DragDistributeButton::Primary {
                state.placed_amounts = new_placed;
            }
        }
    }
    ...
    fn slot_tool_durability_ratio(&self, slot: Slot) -> Option<f32> {
        let tool = slot.tool?;
        if !is_pickaxe_tool(tool) {
            return None;
        }
        ...
        durability = durability.min(max_durability);
        Some((durability as f32 / max_durability as f32).clamp(0.0, 1.0))
    }
...
fn push_drag_placed_amount(entries: &mut Vec<(SlotId, u16)>, slot_id: SlotId, amount: u16) {
    if amount == 0 {
        return;
    }
    if let Some((_, count)) = entries.iter_mut().find(|(id, _)| *id == slot_id) {
        *count = count.saturating_add(amount);
    } else {
        entries.push((slot_id, amount));
    }
}
...
fn paint_slot(
    ...
    tool_durability_ratio: Option<f32>,
) -> egui::Response {
    let (rect, resp) = ui.allocate_exact_size(egui::vec2(size, size), egui::Sense::click());
    let painter = ui.painter();
    ...
    if let Some(ratio) = tool_durability_ratio {
        paint_tool_durability_bar(painter, rect, ratio);
    }

    resp
}

fn paint_tool_durability_bar(painter: &egui::Painter, rect: egui::Rect, ratio: f32) {
    let ratio = ratio.clamp(0.0, 1.0);
    let width = (rect.width() - 6.0).max(2.0);
...
fn is_pickaxe_tool(tool: WoodenTool) -> bool {
    matches!(
        tool,
        WoodenTool::Pickaxe | WoodenTool::StonePickaxe | WoodenTool::IronPickaxe
    )
}
...
fn load_inventory_block_texture(block: Block) -> Option<egui::ColorImage> {
    ...
    if let Some(path) = resolve_inventory_texture_path(block) {
        if let Some(rgba) = decode_rgba_image_from_path(&path) {
            let size = [rgba.width() as usize, rgba.height() as usize];
            let raw = rgba.into_raw();
            return Some(egui::ColorImage::from_rgba_unmultiplied(size, &raw));
    ...
    if let Some(temp) = temporary_inventory_block_texture(block) {
        return Some(temp);
    }

    Some(missing_inventory_texture_image())
}

fn load_inventory_tool_texture(tool: WoodenTool) -> Option<egui::ColorImage> {
    if let Some(path) = item_registry::tool_texture_path(tool) {
        if let Some(rgba) = decode_rgba_image_from_path(&path) {
            let size = [rgba.width() as usize, rgba.height() as usize];
            let raw = rgba.into_raw();
            return Some(egui::ColorImage::from_rgba_unmultiplied(size, &raw));
...
fn missing_inventory_texture_image() -> egui::ColorImage {
    ...
    egui::ColorImage::from_rgba_unmultiplied([16, 16], &rgba)
}

fn temporary_inventory_block_texture(block: Block) -> Option<egui::ColorImage> {
    let mut rgba = vec![0u8; 16 * 16 * 4];
    match block {
        Block::DiamondOre => {
            for y in 0..16usize {
                for x in 0..16usize {
                    let noise = (((x as u32).wrapping_mul(17) ^ (y as u32).wrapping_mul(31) ^ 0x5A) & 7) as u8;
                    let stone = 104u8.saturating_add(noise * 8);
                    let i = (y * 16 + x) * 4;
                    rgba[i] = stone;
                    rgba[i + 1] = stone;
                    rgba[i + 2] = stone;
            ...
            Some(egui::ColorImage::from_rgba_unmultiplied([16, 16], &rgba))
        }
        Block::Diamond => {
            for y in 0..16usize {
                for x in 0..16usize {
            ...
            for &(x, y) in &[(6usize, 5usize), (9, 6), (7, 8), (10, 9), (8, 11)] {
                let i = (y * 16 + x) * 4;
                rgba[i] = 190;
                rgba[i + 1] = 255;
                rgba[i + 2] = 255;
...
fn inventory_texture_aliases(block: Block) -> &'static [&'static str] {
    match block {
        ...
        Block::Diamond => &["diamond", "diamond_gem", "gem_diamond"],
        ...
        Block::DiamondOre => &["diamond_ore", "diamondore", "diamond_ore_stone"],
    }
}
...
fn block_color(block: Block) -> Color32 {
    match block {
        ...
        Block::Diamond => Color32::from_rgb(74, 236, 255),
        ...
        Block::DiamondOre => Color32::from_rgb(96, 168, 182),
    }
}
