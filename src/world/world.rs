...
impl World {
    pub fn new(seed: u32, args: &Args) -> Self {
        ...
        let (gen_tx, gen_task_rx) = mpsc::sync_channel::<(i32, i32, u32)>(64);
        ...
        std::thread::spawn(move || {
            while let Ok((cx, cz, seed)) = gen_task_rx.recv() {
                let tx = Arc::clone(&gen_tx2);
                gen_pool.spawn(move || {
                    let chunk = Chunk::generate(cx, cz, seed);
        ...
        'gen: for r in 0..=(rd + 1) {
            for dx in -r..=r {
                for dz in -r..=r {
                    ...
                    if self.gen_tx.try_send((key.0, key.1, self.seed)).is_ok() {
